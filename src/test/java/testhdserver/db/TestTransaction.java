...
//        testClosingConnectionWithLockedTable(); //不支持锁表查询
//        testConstraintCreationRollback();//不支持表变更选项
//        testConcurrentSelectForUpdate();//不支持锁表查询
//        testForUpdate(); //不支持锁表查询
//        testMergeUsing();//不支持merge
//        testSetTransaction();//不支持这个特性
//        testReferential();//不支持自增id
//        testIsolation(); //duckdb的隔离模式与h2不一样
//        testIsolationLevels();//duckdb的隔离模式与h2不一样
//        testIsolationLevels2();//duckdb的隔离模式与h2不一样
//        testIsolationLevels3();//duckdb的隔离模式与h2不一样
//        testIsolationLevels4();//duckdb的隔离模式与h2不一样
//        testIsolationLevelsCountAggregate2();//duckdb的隔离模式与h2不一样

//        testDelete(); //测试失败，duckdb 执行delete 语句时没有触发乐观锁
...
//        stat1.execute("MERGE INTO TEST KEY(ID) VALUES (1, 1)");
        stat1.execute("INSERT OR REPLACE INTO TEST SELECT 1 AS ID,1 AS V"); //duckdb不支持merge into
...
//        stat1.execute("CREATE TABLE TEST (ID INT PRIMARY KEY, V INT UNIQUE)");
        ...
        conn1.setAutoCommit(false);
//        conn2.createStatement().execute("SET LOCK_TIMEOUT 2000");
        stat1.execute("INSERT OR REPLACE INTO TEST SELECT ID,V FROM ( VALUES (1, 1), (2, 2), (3, 3), (4, 4) ) AS T(ID,V)");
        conn1.commit();
        stat1.execute("UPDATE TEST SET V = 10 - V");
        final Exception[] ex = new Exception[1];
        StringBuilder builder = new StringBuilder("SELECT V FROM TEST ORDER BY V");
        String query = builder.toString();
        final PreparedStatement prep2 = conn2.prepareStatement(query);
        Thread t = new Thread() {
            @Override
...
//                + "SELECT X, FALSE FROM GENERATE_SERIES(1, " + count + ')');
        ...
        stat1.execute("CREATE TABLE TEST(ID INT PRIMARY KEY, \"VALUE\" BOOLEAN)");
        stat1.execute("INSERT OR REPLACE INTO TEST SELECT ID , FALSE AS \"VALUE\" FROM GENERATE_SERIES(1, " + count + ") AS T(ID)");
        ...
        try {
            ...
            System.out.println("exception transastion");
            conn2.rollback();
        }
...
//                + "SELECT X, FALSE FROM GENERATE_SERIES(1, " + count + ')');
        ...
        conn1.commit();
        ResultSet r1 = stat1.executeQuery("SELECT * FROM TEST");
        ResultSet r2 = stat2.executeQuery("SELECT * FROM TEST");
        int a1 =0;
        int a2 =0;
        while(r1.next()) {
            a1++;
        ...
        Thread t = new Thread() {
            ...
            public void run() {
                ...
                r[0] = sum;
                System.out.println("save r0:"+r[0]) ;
            }
        };
        ...
        try {
            ...
            System.out.println("exception delete transation");
            conn2.rollback();
        }
        ...
        System.out.println("sum:" + sum + " r:" + r[0]);
        assertEquals(count, sum + r[0]);
        conn2.close();
        conn1.close();
        server.stop();
...
//        stat.execute("create table master(id int primary key) as select 1");
        ...
        //duckdb不支持同时指定字段约束和select语句指定
        stat.execute("create table master(id int primary key)");
        stat.execute("insert into master select 1 as id");
...
//        stat.execute("create table child2(id int references master(id)) as select 1");
        //duckdb不支持同时指定字段约束和select语句指定
        stat.execute("create table child2(id int references master(id))");
        stat.execute("insert into child2 select 1 as id");


        conn.close();
        conn = getConnection("transaction");
        conn.setAutoCommit(false); //为什么原来这里没有set commit？是没有测试覆盖到吗？
        stat = conn.createStatement();
        //抛出常规异常，还没转换为h2的error code
        assertThrows(
                0, stat).
...
//        s1.execute("create local temporary table a (id int, x BLOB)");
        //jsqlparser 不支持 local tempoarary ,只能tempoarary
        s1.execute("create  temporary table a (id int, x BLOB)");
        c1.commit();
        c1.close();
...
//            stat1.execute("CREATE TABLE TEST(V BIGINT) AS VALUES 1, 2, 3, 4, 5, 18");
            stat1.execute("CREATE TABLE TEST(V BIGINT)");
            stat1.execute("INSERT INTO TEST SELECT V FROM (VALUES (1), (2), (3), (4), (5), (18)) AS T(V)");
//            conn1.setTransactionIsolation(isolationLevel);
            conn1.setAutoCommit(false);
            ...
            stat2.executeUpdate("INSERT INTO TEST SELECT V FROM GENERATE_SERIES(10, 17) AS T(V)");
            testIsolationLevelsCountAggregate(all, simple, uncommitted1);
            stat1.executeUpdate("DELETE FROM TEST WHERE V = 2");
            stat1.executeUpdate("INSERT INTO TEST SELECT V FROM GENERATE_SERIES(6, 9) AS T(V)");

...
//    private static final String CATALOG = "METADATA";
    ...
    private static final String SCHEMA = "metadata";
        private static final String CATALOG = "system";
    ...
    public void test() throws SQLException {
        deleteDb("metaData");
//        Server server = createServer(4007);
//        testUnwrap();
...
//        assertTrue(conn.isReadOnly() == meta.isReadOnly());
    ...
    private void testGeneral() throws SQLException {
        ...
        stat.execute("CREATE TABLE main.TEST(ID INT PRIMARY KEY, NAME VARCHAR(255))");
        stat.execute("CREATE INDEX IDXNAME ON main.TEST(NAME)");
        ...
        //duckdb  有多个可用的catalog
//        assertFalse(rs.next());

        rs = meta.getSchemas();
        rs.next();
        assertEquals("INFORMATION_SCHEMA".toLowerCase(), rs.getString("TABLE_SCHEM"));
        rs.next();
...
//        assertEquals("SYNONYM", rs.getString("TABLE_TYPE"));
        ...
        rs = meta.getTables(null, SCHEMA,
                null, new String[] { "BASE TABLE" });
        assertNull(rs.getStatement());
        rs.next();
        assertEquals("TEST", rs.getString("TABLE_NAME"));
        assertFalse(rs.next());

        //INFORMATION_SCHEMA暂时没搞懂 INFORMATION_SCHEMA，后续数据库勘查
//        rs = meta.getTables(null, "INFORMATION_SCHEMA".toLowerCase(), null, new String[] { "BASE TABLE", "VIEW","LOCAL TEMPORARY" });
...
//        assertFalse(rs.next());
//
        //duckdb jdbc暂时不支持，后续手动用sql查 db元数据 实现该功能
//        rs = meta.getIndexInfo(null, null, "TEST", false, false);
//        rs.next();

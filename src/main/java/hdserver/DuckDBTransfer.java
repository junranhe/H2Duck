 ...
 */
public final class DuckDBTransfer {
...
//     * @param s the socket
//     */
//    public DuckDBTransfer(Session session, Socket s) {
//        this.session = session;
//        this.socket = s;
//    }

    public DuckDBTransfer( Socket s) {
        ...
        if (socket != null) {
            in = new DataInputStream(
                    new BufferedInputStream(
                            socket.getInputStream(), DuckDBTransfer.BUFFER_SIZE));
            out = new DataOutputStream(
                    new BufferedOutputStream(
                            socket.getOutputStream(), DuckDBTransfer.BUFFER_SIZE));
        }
    }
     ...
     * @throws IOException on failure
     */
    public DuckDBTransfer writeBoolean(boolean x) throws IOException {
     ...
     * @throws IOException on failure
     */
    public DuckDBTransfer writeByte(byte x) throws IOException {
        out.writeByte(x);
        return this;
    }
     ...
     * @throws IOException on failure
     */
    private DuckDBTransfer writeShort(short x) throws IOException {
        out.writeShort(x);
        return this;
    }
     ...
     * @throws IOException on failure
     */
    public DuckDBTransfer writeInt(int x) throws IOException {
        out.writeInt(x);
        return this;
    }
     ...
     * @throws IOException on failure
     */
    public DuckDBTransfer writeLong(long x) throws IOException {
        out.writeLong(x);
        return this;
    }
     ...
     * @throws IOException on failure
     */
    private DuckDBTransfer writeDouble(double i) throws IOException {
        out.writeDouble(i);
        return this;
    }
     ...
     * @return itself
     */
    private DuckDBTransfer writeFloat(float i) throws IOException {
        out.writeFloat(i);
        return this;
    }
     ...
     * @throws IOException on failure
     */
    public DuckDBTransfer writeString(String s) throws IOException {
        if (s == null) {
            out.writeInt(-1);
        } else {
     ...
     * @throws IOException on failure
     */
    public DuckDBTransfer writeBytes(byte[] data) throws IOException {
        if (data == null) {
            writeInt(-1);
        } else {
     ...
     * @throws IOException on failure
     */
    public DuckDBTransfer writeBytes(byte[] buff, int off, int len) throws IOException {
    ...
    public byte[] readBytes() throws IOException {
        ...
        byte[] b = org.h2.util.Utils.newBytes(len);
        in.readFully(b);
        return b;
    }
     ...
     * @throws IOException on failure
     */
    public DuckDBTransfer writeTypeInfo(TypeInfo type) throws IOException {
    ...
    private void writeTypeInfoRow(TypeInfo type) throws IOException {
        Set<Entry<String, TypeInfo>> fields = ((ExtTypeInfoRow) type.getExtTypeInfo()).getFields();
        writeInt(fields.size());
        for (Entry<String, TypeInfo> field : fields) {
    ...
    public void writeResultSetValue(String typeName, ResultSet rs, int index) throws IOException, SQLException {
        if (typeName == "NULL") {
            writeInt(NULL);
            ...
            writeInt(BOOLEAN);
            writeBoolean(rs.getBoolean(index));

        }else if (typeName == "INTEGER") {
            writeInt(INTEGER);
            writeInt(rs.getInt(index));
        }else if (typeName == "BIGINT") {
            writeInt(BIGINT);
            writeLong(rs.getLong(index));
            ...
            writeInt(DOUBLE);
            writeDouble(rs.getDouble(index));
        } else if (typeName == "CHAR") {
            assert false;
        } else if (typeName == "VARCHAR") {
            writeInt(VARCHAR);
            writeString(rs.getString(index));
        } else {
            assert false;
        }
    }
...
//                return ValueEnumBase.get(readString(), ordinal);
                throw DbException.getUnsupportedException("readValue type:ENUM unsuported");
            }
            case INTEGER:
            ...
            case BLOB: {
                throw DbException.getUnsupportedException("readValue type:BLOB unsuported");
...
//                return v;
            ...
            case CLOB: {
                throw DbException.getUnsupportedException("readValue type:CLOB unsuported");
...
//                return v;
            ...
            case ARRAY: {
//                throw DbException.getUnsupportedException("readValue type:ARRAY unsuported");
                int len = readInt();
                if (len < 0) {
...
//                    return ValueArray.get(elementType, readArrayElements(len, elementType), session);
                    return ValueArray.get(elementType, readArrayElements(len, elementType), null);

                }
                    return ValueArray.get(readArrayElements(len, null), null);
//                return ValueArray.get(readArrayElements(len, null), session);
     ...
     * @throws IOException on failure
     */
    public DuckDBTransfer writeRowCount(long rowCount) throws IOException {
...
//    }
    ...
    public void verifyLobMac(byte[] hmac, long lobId) {
        ...
        if (!org.h2.util.Utils.compareSecure(hmac,  result)) {
